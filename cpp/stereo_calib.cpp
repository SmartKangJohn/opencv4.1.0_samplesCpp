...
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    for( i = j = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            string imgPath = "0_data/data/" + filename;
        ...
        if( k == 2 )
        {
            string imgPath = "0_data/data/" + imagelist[i * 2];
            goodImageList.push_back(imgPath);
            imgPath = "0_data/data/" + imagelist[i * 2 + 1];
            goodImageList.push_back(imgPath);
            j++;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    string imagelistfn;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|9|}{h|6|}{s|30.0|}{nr||}{help||}{@input|0_data/data/stereo_calib.xml|}");  //s:mm
    if (parser.has("help"))
        return print_help();
